...
bool UdpCom::writeTo(std::span<const uint8_t> msg, const std::string& ip, uint16_t port) {
    if (!initialized || !sock.is_open()) {
        return false;
    }

    SSIZE_T bytesSent = sock.send_to(msg.data(), msg.size(), sockpp::inet_address(ip, port));
